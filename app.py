...
from speech_to_text import SpeechToTextManager # Import SpeechToTextManager for audio transcription
...
if "stt_manager" not in st.session_state:
    try:
        ...
        logger.info("SpeechToTextManager initialized in session state.")
    except Exception as e:
        st.error(f"Failed to initialize Speech-to-Text service: {e}")
        logger.critical(f"Failed to initialize SpeechToTextManager: {e}", exc_info=True)
...
if text_prompt: # st.chat_input returns a non-empty string on submission
    ...
    # The line st.session_state.audio_input_key += 1 was removed as requested.
...
# This 'elif' ensures audio is only processed if text_prompt was not just submitted.
elif audio_file_uploaded is not None and audio_file_uploaded.size > 0:
    with st.spinner("Transcribing audio..."):
        try:
            transcribed_text = st.session_state.stt_manager.transcribe_audio(audio_file_uploaded)
            if transcribed_text:
